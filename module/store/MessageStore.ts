...
class MessageStore {
	...
	sendMessage(message: AgoraChat.MessageBody) {
		...
		this.rootStore.client
			...
			.then((data: { serverMsgId: string }) => {
				...
				this.message.byId[data.serverMsgId] =
					this.message.byId[message.id];
				// @ts-ignore
				this.message.byId[message.id].status = 'sent';
				// @ts-ignore
				this.message.byId[message.id].mid = data.serverMsgId;
				// @ts-ignore
				const i = this.message[chatType][to].indexOf(
					this.message.byId[message.id]
				);
				// @ts-ignore
	...
	updateMessageStatus(msgId: string, status: string) {
		setTimeout(() => {
			...
			// @ts-ignore
			const i = this.message[msg.chatType][conversationId].indexOf(
				this.message.byId[msg.id]
			);
			// @ts-ignore
