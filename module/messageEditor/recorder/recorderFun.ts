...
const HZRecorder = function (
  ...
  const interpolateArray = (data: Float32Array, newSampleRate: number, oldSampleRate: number) => {
    var fitCount = Math.round(data.length * (newSampleRate / oldSampleRate));
    var newData = new Array();
    var springFactor = new Number((data.length - 1) / (fitCount - 1));
    newData[0] = data[0]; // for new allocation
    for (var i = 1; i < fitCount - 1; i++) {
      var tmp = i * (springFactor as number);
      var before = new Number(Math.floor(tmp)).toFixed();
      var after = new Number(Math.ceil(tmp)).toFixed();
      var atPoint = tmp - (before as any as number);
      newData[i] = linearInterpolate(
        data[before as any as number],
        data[after as any as number],
        atPoint,
      );
    }
  ...
  // 开始录音
  // @ts-ignore
  (this as typeof HZRecorder).start = function () {
    audioInput.connect(recorder);
    recorder.connect(context.destination);
  };
  this.isEmptyData = function (d: string | any[]) {
  ...
  // 回放
  this.play = function (audio: HTMLAudioElement) {
...
HZRecorder.setErrorInfoText = (errorMessage: any) => {
  //@ts-ignore
  HZRecorder.errorMessage = errorMessage;
};

// 获取录音机
HZRecorder.get = function (callback: (arg0: any, arg1: MediaStream) => void, config?: object) {
  if (callback) {
