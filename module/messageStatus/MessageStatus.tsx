...
const MessageStatus = (props: MessageStatusProps) => {
	...
	switch (status) {
		case 'sending':
			statusNode =
				type == 'icon' ? (
					<img alt="sending" src={sending}></img>
				) : (
					'发送中'
				);
			break;
		case 'sent':
			statusNode =
				type == 'icon' ? <img alt="sent" src={sent}></img> : '已发送';
			break;
		case 'received':
			statusNode =
				type == 'icon' ? (
					<img alt="received" src={received}></img>
				) : (
					'发送成功'
				);
			break;
		case 'read':
			statusNode =
				type == 'icon' ? <img alt="read" src={read}></img> : '已读';
			break;
		case 'unread':
